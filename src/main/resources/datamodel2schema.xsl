...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:xalan="http://xml.apache.org/xalan" xmlns:xs="http://www.w3.org/2001/XMLSchema"
      ...
      <xs:documentation>
        <xsl:choose>
          <xsl:when test="string-length($plugin.groupId)&gt;0">
            <xsl:value-of
              select="concat('Generated by ',$plugin.groupId,':',$plugin.artifactId,':', $plugin.version)" />
          </xsl:when>
          <xsl:otherwise>
  ...
  <xsl:template match="element[@type='datetext']" mode="metadata">
    <xsl:apply-templates select="." mode="enclosing">
      <xsl:with-param name="class" select="'MCRMetaDateLangText'" />
    </xsl:apply-templates>
    <xsl:apply-templates select="." mode="inner">
      <xsl:with-param name="class" select="'MCRMetaDateLangText'" />
      <xsl:with-param name="containsText" select="true()" />
  ...
  <xsl:template match="element[@type='address']" mode="metadata">
    <xsl:apply-templates select="." mode="enclosing">
      <xsl:with-param name="class" select="'MCRMetaAddress'" />
    </xsl:apply-templates>
    <xsl:variable name="innerSchema">
      <xs:any>
